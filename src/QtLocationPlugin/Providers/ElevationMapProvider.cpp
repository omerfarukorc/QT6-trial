 ...
 *
 * License for the COPERNICUS dataset hosted on
...
int CopernicusElevationProvider::long2tileX(double lon, int z) const {
  Q_UNUSED(z)
  return static_cast<int>(floor((lon + 180.0) / 1.0));
}

int CopernicusElevationProvider::lat2tileY(double lat, int z) const {
  Q_UNUSED(z)
  return static_cast<int>(floor((lat + 90.0) / 1.0));
}
...
QGCTileSet CopernicusElevationProvider::getTileCount(
    int zoom, double topleftLon, double topleftLat, double bottomRightLon,
  ...
  set.tileCount =
      (static_cast<quint64>(set.tileX1) - static_cast<quint64>(set.tileX0) +
       1) *
...
QByteArray
CopernicusElevationProvider::serialize(const QByteArray &image) const {
  Q_UNUSED(image)
  return QByteArray();
}
