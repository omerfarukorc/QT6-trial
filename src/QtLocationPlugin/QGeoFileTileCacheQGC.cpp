...
QGC_LOGGING_CATEGORY(QGeoFileTileCacheQGCLog,
                     "qgc.qtlocationplugin.qgeofiletilecacheqgc")
...
uint32_t QGeoFileTileCacheQGC::_getMemLimit(const QVariantMap &parameters) {
  ...
  if (parameters.contains(QStringLiteral("mapping.cache.memory.size"))) {
    bool ok = false;
    memLimit = parameters.value(QStringLiteral("mapping.cache.memory.size"))
                   .toString()
                   .toUInt(&ok);
    if (!ok) {
      memLimit = 0;
...
void QGeoFileTileCacheQGC::cacheTile(const QString &type, int x, int y, int z,
                                     const QByteArray &image,
                                     const QString &format, qulonglong set) {
  const QString hash = UrlFactory::getTileHash(type, x, y, z);
  cacheTile(type, hash, image, format, set);
}

void QGeoFileTileCacheQGC::cacheTile(const QString &type, const QString &hash,
                                     const QByteArray &image,
                                     const QString &format, qulonglong set) {
  if (true) {
...
QString QGeoFileTileCacheQGC::_getCachePath(const QVariantMap &parameters) {
  ...
  if (parameters.contains(QStringLiteral("mapping.cache.directory"))) {
    cacheDir =
        parameters.value(QStringLiteral("mapping.cache.directory")).toString();
  } else {
    cacheDir = _cachePath + QLatin1String("/providers");
    if (!QFileInfo::exists(cacheDir)) {
      if (!QDir::root().mkpath(cacheDir)) {
        qCWarning(QGeoFileTileCacheQGCLog)
...
void QGeoFileTileCacheQGC::_initCache() {
  ...
  if (!_cachePath.isEmpty()) {
    ...
    qCCritical(QGeoFileTileCacheQGCLog)
        << "Could not find suitable map cache directory.";
  }

  if (_cacheWasReset) {
    qgcApp()->showAppMessage(
        tr("The Offline Map Cache database has been upgraded. "
